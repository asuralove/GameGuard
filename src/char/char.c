...
int char_memitemdata_to_sql(const struct item items[], int max, int id, int tableswitch) {
	...
	StringBuf_Init(&buf);
	StringBuf_AppendStr(&buf, "SELECT `id`, `nameid`, `amount`, `equip`, `identify`, `refine`, `attribute`, `expire_time`, `bound`, `unique_id`, `favorite`");
		...
		for( i = 0; i < max; ++i )
		{
			...
			&&  items[i].card[3] == item.card[3]
				...
				{ // Differences founds, updated required
					StringBuf_Clear(&buf);
					StringBuf_Printf(&buf, "UPDATE `%s` SET `amount`='%d', `equip`='%d', `identify`='%d', `refine`='%d',`attribute`='%d', `expire_time`='%u', `bound`='%d', `unique_id`='%"PRIu64"', `favorite`='%d'",
						tablename, items[i].amount, items[i].equip, items[i].identify, items[i].refine, items[i].attribute, items[i].expire_time, items[i].bound, items[i].unique_id, items[i].favorite);
					for( j = 0; j < MAX_SLOTS; ++j )
	...
	StringBuf_Clear(&buf);
	StringBuf_Printf(&buf, "INSERT INTO `%s`(`%s`, `nameid`, `amount`, `equip`, `identify`, `refine`, `attribute`, `expire_time`, `bound`, `unique_id`, `favorite`", tablename, selectoption);
	for( j = 0; j < MAX_SLOTS; ++j )
	...
	for( i = 0; i < max; ++i )
	{
		...
		StringBuf_Printf(&buf, "('%d', '%hu', '%d', '%d', '%d', '%d', '%d', '%u', '%d', '%"PRIu64"', '%d'",
			id, items[i].nameid, items[i].amount, items[i].equip, items[i].identify, items[i].refine, items[i].attribute, items[i].expire_time, items[i].bound, items[i].unique_id, items[i].favorite);
		for( j = 0; j < MAX_SLOTS; ++j )
...
int char_mmo_char_fromsql(uint32 char_id, struct mmo_charstatus* p, bool load_everything) {
	...
	if( SQL_ERROR == SqlStmt_Prepare(stmt, "SELECT "
		...
		"`unban_time`,`font`,`uniqueitem_counter`,`sex`,`hotkey_rowshift`,`playtime`,`faction_id`"
	...
	||	SQL_ERROR == SqlStmt_BindColumn(stmt, 58, SQLDT_UINT,   &p->playtime, 0, NULL, NULL)
	||	SQL_ERROR == SqlStmt_BindColumn(stmt, 59, SQLDT_INT,	&p->faction_id, 0, NULL, NULL)
	)
	{
		SqlStmt_ShowDebug(stmt);
		SqlStmt_Free(stmt);
	...
	StringBuf_AppendStr(&msg_buf, " rentstorage");
	...
	StringBuf_AppendStr(&msg_buf, " pvprank");
...
{	
	char filename[128], esc_name[NAME_LENGTH*2+1];
	struct mmo_charstatus cp;
	struct item *i_data;
...
void char_read_fame_list(void)
{
	...
	for( i = 0; i < fame_list_size_smith && SQL_SUCCESS == Sql_NextRow(sql_handle); ++i )
	{
		...
		Sql_GetData(sql_handle, 2, &data, &len); memcpy(smith_fame_list[i].name, data, zmin(len, NAME_LENGTH));
	}
	...
	for( i = 0; i < fame_list_size_chemist && SQL_SUCCESS == Sql_NextRow(sql_handle); ++i )
	{
		...
		Sql_GetData(sql_handle, 2, &data, &len); memcpy(chemist_fame_list[i].name, data, zmin(len, NAME_LENGTH));
	}
	...
	for( i = 0; i < fame_list_size_taekwon && SQL_SUCCESS == Sql_NextRow(sql_handle); ++i )
	{
		...
		Sql_GetData(sql_handle, 2, &data, &len); memcpy(taekwon_fame_list[i].name, data, zmin(len, NAME_LENGTH));
	}
	...
	for( i = 0; i < fame_list_size_bgrank && SQL_SUCCESS == Sql_NextRow(sql_handle); ++i )
	{
		...
		Sql_GetData(sql_handle, 2, &data, &len); memcpy(bgrank_fame_list[i].name, data, zmin(len, NAME_LENGTH));
	}
	Sql_FreeResult(sql_handle);
	...
	for( i = 0; i < fame_list_size_bg && SQL_SUCCESS == Sql_NextRow(sql_handle); ++i )
	{
		...
		Sql_GetData(sql_handle, 2, &data, &len); memcpy(bg_fame_list[i].name, data, zmin(len, NAME_LENGTH));
	}
	Sql_FreeResult(sql_handle);
}
