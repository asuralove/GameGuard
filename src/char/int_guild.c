...
int guild_calcinfo(struct guild *g)
{
	...
	g->max_member = charserv_config.guild_base_members + guild_checkskill(g, GD_EXTENSION) * charserv_config.guild_add_members;
	if(g->max_member > MAX_GUILD)
	{
...
int mapif_parse_CreateGuild(int fd,uint32 account_id,char *name,struct guild_member *master)
{
	...
	g->max_member = charserv_config.guild_base_members;
