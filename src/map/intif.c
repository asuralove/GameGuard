...
static const int packet_len_table[]={
	...
	-1,-1, 7, 7,  7,11, 8,-1,  0, 0,-1, 7, -1, 0,  0, 0, //0x3850  Auctions [Zephyrus] + Achievement
	...
	if(num_received == 0) {
		...
		for(i = 0; i < num_received; i++) {
			...
			else // Insert at the end
			{
				...
				achievement_validate_quest(sd,received[i].quest_id); // Maybe new achievements implemented of type AT_QUEST
			}
			sd->num_quests++;
		}
...
// Achievement System
	...
	case 0x385c:	intif_parse_Auction_data(fd); break;
