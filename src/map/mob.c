...
int mob_setdelayspawn(struct mob_data* md, struct map_session_data* sd)
{
	...
	if( sd && battle_config.mob_graveyard && status_get_class_(&md->bl) == CLASS_BOSS && spawntime >= 60000 )
...
static int mob_delay_item_drop(int tid, unsigned int tick, int id, intptr_t data)
{
	...
	while (ditem) {
		...
		map_addflooritem(&ditem->item_data,ditem->item_data.amount,
			...
			list->first_charid,list->second_charid,list->third_charid,4,ditem->mob_id,0);
		ditem_prev = ditem;
		ditem = ditem->next;
...
static void mob_item_drop(struct mob_data *md, struct item_drop_list *dlist, struct item_drop *ditem, int loot, int drop_rate, bool flag)
{
	...
	test_autoloot = sd 
		&& ( drop_rate <= sd->aloot.rate || i < MAX_AUTOLOOTID || pc_isautolooting(sd, ditem->item_data.nameid) )
...
int mob_dead(struct mob_data *md, struct block_list *src, int type)
{
	...
	if(!(type&2) && //No exp
		...
		for(i = 0; i < DAMAGELOG_SIZE && md->dmglog[i].id; i++) {
			...
			if (map[m].flag.nobaseexp || !md->db->base_exp)
				...
				exp = apply_rate(exp, bonus + battle_config.base_exp_rate_bonus + md->option.exp_boost);
			...
			if (map[m].flag.nojobexp || !md->db->job_exp || md->dmglog[i].flag == MDLF_HOMUN) //Homun earned job-exp is always lost.
				...
				exp = apply_rate(exp, bonus + battle_config.job_exp_rate_bonus + md->option.exp_boost);
	...
	{ // Item Drop
		...
		for (i = 0; i < MAX_MOB_DROP; i++) {
			...
			if( sd ){
				...
				if( rd && rd->bonus_drop )
				{
					drop_rate += (int)(0.5 + (drop_rate * rd->bonus_drop) / 100);
					drop_rate = min(drop_rate,10000); //cap it to 100%
				}
				if( pc_isPremium(sd) && battle_config.premium_dropboost )
				{
		...
		{ // process script-granted extra drop bonuses
			...
			for (i = 0; i < ARRAYLENGTH(sd->add_drop); i++) {
					...
					(sd->add_drop[i].class_ == CLASS_ALL || sd->add_drop[i].class_ == status->class_)) //Matched class
				{
					...
					mob_item_drop(md, dlist, mob_setdropitem(dropid,1,md->mob_id), 0, drop_rate, aikillonly);
				}
			}
		...
		if (md->lootitems) {
			for (i = 0; i < md->lootitem_count; i++)
				mob_item_drop(md, dlist, mob_setlootitem(&md->lootitems[i], md->mob_id), 1, 10000, aikillonly);
		}
		if (dlist->item) //There are drop items.
			add_timer(tick + (!battle_config.delay_battle_damage?500:0), mob_delay_item_drop, 0, (intptr_t)dlist);
		else //No drops
			ers_free(item_drop_list_ers, dlist);
	} else if (md->lootitems && md->lootitem_count && !md->option.no_expdrop) {	//Loot MUST drop!
		struct item_drop_list *dlist = ers_alloc(item_drop_list_ers, struct item_drop_list);
		dlist->m = md->bl.m;
		dlist->x = md->bl.x;
		dlist->y = md->bl.y;
	...
	if( mvp_sd && md->db->mexp > 0 && !md->special_state.ai && !md->option.no_expdrop ) {
		...
		if( !(map[m].flag.nomvploot || type&1) ) {
			...
			for(i = 0; i < MAX_MVP_DROP; i++) {
				...
				if((temp = pc_additem(mvp_sd,&item,1,LOG_TYPE_PICKDROP_PLAYER)) != 0) {
					...
					map_addflooritem(&item,1,mvp_sd->bl.m,mvp_sd->bl.x,mvp_sd->bl.y,mvp_sd->status.char_id,(second_sd?second_sd->status.char_id:0),(third_sd?third_sd->status.char_id:0),1,0,0);
				}
		...
		pc_setglobalreg(mvp_sd,add_str("MVPRank"),mvp_sd->custom_data.MVPKiller);
	}
	...
	if( !rebirth ) { // Only trigger event on final kill
		...
		if( sd ) {
			...
			achievement_validate_mob(sd,md->mob_id);
			...
			{ // Hunting Missions [Zephyrus]
				...
				{ // Search into the party if someone is hunting the killed mob
					for( i = 0; i < MAX_PARTY; i++ )
					{
						...
						ARR_FIND(0, 5, c, pl->hunting[c].mob_id == md->mob_id && pl->hunting[c].count > 0);
						if( c < 5 ) break;
					}
				...
				{
					ARR_FIND(0, 5, c, sd->hunting[c].mob_id == md->mob_id && sd->hunting[c].count > 0);
					if( c < 5 ) pl = sd;
				}
				...
				if( pl != NULL && c < 5 && pl->hunting[c].count > 0 )
				{
					...
					pc_setglobalreg(pl, add_str(varname), pl->hunting[c].count);

					if( pl->hunting[c].count > 0 )
						sprintf(output, "[ Hunting Mission : %d '%s' remaining ]", pl->hunting[c].count, mob_db(md->mob_id)->jname);
					else
						sprintf(output, "[ Hunting Mission : '%s' hunting completed ]", mob_db(md->mob_id)->jname);
...
int mob_clone_spawn(struct map_session_data *sd, int16 m, int16 x, int16 y, const char *event, int master_id, enum e_mode mode, int flag, unsigned int duration)
{
	...
	for (i=0,j = MAX_SKILL_TREE-1;j>=0 && i< MAX_MOBSKILL ;j--) {
		...
		if (!skill_id || !(sk_idx = skill_get_index(skill_id)) || sd->status.skill[sk_idx].lv < 1 ||
			...
			skill_get_nocast(skill_id)&16 || !skill_get_blocked(skill_id)
		)
			continue;
...
static bool mob_parse_dbrow(char** str)
{
	...
	for(i = 0; i < MAX_MVP_DROP; i++) {
		...
		db->mvpitem[i].p = 0;
		db->mvpitem[i].base_p = 0; // eAmod Codes
	}
	...
	for(i = 0; i < MAX_MOB_DROP; i++) {
		...
		if( db->dropitem[i].nameid ){
			if( itemdb_search( db->dropitem[i].nameid ) ){
				db->dropitem[i].p = db->dropitem[i].base_p = atoi(str[k+1]); // eAMod Codes
				continue;
			}else{
		...
		db->dropitem[i].p = 0;
		db->dropitem[i].base_p = 0; // eAmod Codes
	}
	...
	for( i = 0; i <= MAX_MOB_DB; i++ ){
		...
		for( j = 0; j < MAX_MVP_DROP; j++ ){
			...
			if( rate ){
				...
				if( !id ){
					...
					mob->mvpitem[j].p = 0;
					mob->mvpitem[j].base_p = 0; // eAmod Codes
					continue;
				}
		...
		for( j = 0; j < MAX_MOB_DROP; j++ ){
			...
			if( !id ){
				...
				mob->dropitem[j].p = 0;
				mob->dropitem[j].base_p = 0; // eAmod Codes
				continue;
			}
