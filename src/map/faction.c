...
const int lang_pow[LANG_SYS_MAX] = { 0x1, 0x2, 0x4, 0x8, 0x10, 0x20, 0x40, 0x80, 0x100, 0x200, 0x400, 0x800, 0x1000, 0x2000, 0x4000, 0x8000 };
...
void faction_db_load(bool clear)
{
	...
	while( fgets(line,sizeof(line),fp) )
	{
		...
		// Read faction_id,Name,def_ele,ele_lv,size,race,fontcolor,aura_id
		for( i = 0; i < 10; ++i )
		{
			str[i] = p;
			p = strchr(p,';');
		...
		fd->aura_id = atoi(str[9]);

		fd->script = parse_script(str[10],path,lines,0);
